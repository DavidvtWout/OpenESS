...
from open_ess.frontend.dependencies import BatterySystemsDep, Database, PriceConfigDep, TimeseriesDep

from .util import TimeSeries, data_to_timeseries, find_full_battery_cycles, query_result_to_timeseries

if TYPE_CHECKING:
    from open_ess.database import DatabaseConnection
    from open_ess.timeseries import QueryResult, TimeseriesBackend
...
async def get_power_flow(
    ...
    return await _get_power_flow_legacy(db, battery_systems)
...
async def _get_power_flow_legacy(db: "DatabaseConnection", battery_systems: list) -> PowerFlowData:
